...
final class ValidateService: ValidateServiceProtocol {
    func getValidateCode(validateCodeModel: ValidateCodeModel?, completion: @escaping (ValidateModel?, Error?) -> Void) {
        ...
        NetworkManager.manager.request(ServiceBaseUrl.baseUrl + EndPointValidate.validate.rawValue, method: .get, parameters: ["empresa" : "258", "celular" : validateCodeModel?.celular ?? "", "clave" : validateCodeModel?.clave ?? ""]).responseJSON { response in
    ...
    func getRequestSMS(requestCodeModel: RequestCodeModel?, completion: @escaping (String?, Error?) -> Void) {
        ...
        NetworkManager.manager.request(ServiceBaseUrl.baseUrl + EndPointValidate.request.rawValue, method: .get, parameters: ["empresa" : "258", "nombre" : requestCodeModel?.nombre ?? "" , "celular" : requestCodeModel?.celular ?? ""]).responseString { response in
